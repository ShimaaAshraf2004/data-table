...
f2 = (action, customer)=>{
  ...
  customerForm.classList = `fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-[30px] rounded-lg w-[90%] max-w-[500px] shadow-2xl z-999 my-transition`
  customerForm.append(f3(action), f4(action, customer))
  const overlay = f1()
...
f4 = (action, customer)=>{
  ...
  container.innerHTML = `
  <label for="name" class="text-[#2c3e50] font-bold text-[20px]">Name:
  <input
    type="text"
    id="name"
    class="input-name w-full px-3 py-2 rounded-sm border border-solid border-[#eee] outline-none focus:border-[blue]"
